...
- Individual file downloads have no size ceiling at all. Files already on disk are handed to the web server to serve rather than streamed through PHP, which also gives them resume support. This fixes single-video downloads over 1 GB from client galleries, which were silently truncated before.
