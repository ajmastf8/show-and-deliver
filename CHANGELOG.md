...
- The share link is a real download page: one button for everything, your message, and a list of the individual files so a client who only wants one clip doesn't have to pull down the whole archive.
- Transfers past the 4 GB zip limit now use Zip64, so a delivery of any size stays a single download. Splitting into numbered parts is still available for anyone who wants it (`PACKAGE_PART_MB` in `.env`) but is off by default.
