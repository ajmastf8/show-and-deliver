  ...
  await t.sendMail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        ...
        <p><strong>${escapeHtml(comment.name)}</strong> commented on <strong>${escapeHtml(video.title)}</strong>${comment.timestamp > 0 ? ` at <span style="color: #0019ff;">${formatTimestamp(comment.timestamp)}</span>` : ''}</p>
...
async function sendReviewSummary({ gallery, videos, comments, reviewerName, videoTitle, videoId }) {
  ...
  for (const [videoId, vidComments] of Object.entries(commentsByVideo)) {
    ...
    const isPhoto = video && video.type === 'photo';
      ...
      if (isPhoto || c.timestamp === 0) {
        ...
        htmlComments += `
          <div style="padding: 8px 0; border-bottom: 1px solid #f0f0f0;">
            <span style="color: #5f5f5f; font-size: 13px;">${escapeHtml(c.text)}</span>
          </div>`;
      } else {
