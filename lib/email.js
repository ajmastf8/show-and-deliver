...
async function sendReviewSummary({ gallery, videos, comments, reviewerName, videoTitle }) {
  const config = getEmailConfig();
  ...
  for (const [videoId, vidComments] of Object.entries(commentsByVideo)) {
    ...
    htmlComments += `<h3 style="color: #333; margin: 20px 0 8px; font-size: 15px;">${title}</h3>`;
    if (filename) {
      htmlComments += `<p style="color: #8e8e8e; font-size: 12px; margin: 0 0 8px;">File: ${filename}</p>`;
    }
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <h2 style="color: #0019ff;">Comments on ${subjectVideo}</h2>
        <p><strong>${reviewerName}</strong> left ${comments.length} comment${comments.length !== 1 ? 's' : ''} on <strong>${gallery.name}</strong>.</p>
        ${htmlComments}
        <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 24px 0;">
